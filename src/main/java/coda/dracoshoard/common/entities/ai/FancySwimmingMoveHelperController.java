package coda.dracoshoard.common.entities.ai;
...
public class FancySwimmingMoveHelperController extends MoveControl {
    private final PathfinderMob entity;

    public FancySwimmingMoveHelperController(PathfinderMob entity) {
        super(entity);
        this.entity = entity;
    }
    ...
    public void tick() {
        ...
        if (this.operation == MoveControl.Operation.MOVE_TO && !this.entity.getNavigation().isDone()) {
            ...
            if (d3 < (double)2.5000003E-7F) {
                ...
                this.entity.yRotO = this.rotlerp(this.entity.getYRot(), f, 10.0F);
                this.entity.yBodyRot = this.entity.getYRot();
                this.entity.yHeadRot = this.entity.getYRot();
                float f1 = (float)(this.speedModifier * this.entity.getAttributeValue(Attributes.MOVEMENT_SPEED) * 0.1D);
                if (this.entity.isInWater()) {
                    this.entity.setSpeed(f1 * 0.02F);
                    float f2 = -((float)(Mth.atan2(d1, Math.sqrt(d0 * d0 + d2 * d2)) * (double)(180F / (float)Math.PI)));
                    f2 = Mth.clamp(Mth.wrapDegrees(f2), -85.0F, 85.0F);
                    this.entity.xRotO = this.rotlerp(this.entity.getXRot(), f2, 5.0F);
